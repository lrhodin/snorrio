...
export function buildCacheProvenanceManifest(
  level: CacheLevel,
  ref: string,
  options: { episodesDir?: string; lineageIndex?: SessionLineageIndex } = {},
  ...
  let dates: string[] = [];
  try { dates = readdirSync(episodesDir).filter((date) => /^\d{4}-\d{2}-\d{2}$/.test(date) && dateMatches(level, ref, date)).sort(); }
  catch {}
  ...
  for (const date of dates) {
    ...
    for (const file of files) {
      ...
      const sessionId = scalar(parsed.fields.get("session_id")) ?? fileId;
...
export function ensureCacheProvenanceManifest(
  level: CacheLevel,
  ref: string,
  options: { cacheDir?: string; episodesDir?: string; lineageIndex?: SessionLineageIndex; force?: boolean } = {},
): CacheProvenanceManifest {
  const cacheDir = options.cacheDir ?? join(SNORRIO_HOME, "cache");
